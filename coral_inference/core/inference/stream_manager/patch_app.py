...
TERMINATION_GRACE_PERIOD = float(os.getenv("STREAM_MANAGER_TERMINATION_GRACE_PERIOD", "5.0"))
...
def patched_get_response_ignoring_thrash(responses_queue: Queue, matching_request_id: str) -> dict:
    ...
    while retry_count < max_retries and time.time() - start_time < QUEUE_TIMEOUT * 2:
        try:
            ...
            logger.warning(f"Dropping response for request_id={response[0]} (expected {matching_request_id})")
            
        except TimeoutError:
            retry_count += 1
            logger.warning(f"Retry {retry_count}/{max_retries} for request {matching_request_id}")
            continue
...
def patched_execute_termination(
    ...
    with app.PROCESSES_TABLE_LOCK:
        ...
        for pipeline_id in pipeline_ids:
            PIPELINE_HEALTH[pipeline_id] = {'marked_for_removal': True, 'failures': 0, 'last_check': time.time()}
        ...
        for pipeline_id in pipeline_ids:
            ...
            try:
                ...
                if join_thread.is_alive():
                    logger.error(f"Pipeline {pipeline_id} failed to join within timeout")
                else:
...
def perform_safe_health_check(pipeline_id: str, managed_pipeline: app.ManagedInferencePipeline) -> bool:
    ...
    try:
        ...
        if (
            REPORT_KEY in response
            and SOURCES_METADATA_KEY in response[REPORT_KEY]
        ):
            ...
            if all_sources_statuses and all_sources_statuses.issubset({StreamState.ENDED, StreamState.ERROR}):
                logger.info(f"All sources depleted in pipeline {pipeline_id}, scheduling termination")
                # Schedule async termination
                threading.Thread(
                    target=lambda: terminate_pipeline_async(pipeline_id),
                    daemon=True
                ).start()
                return False
            ...
            for pipeline_id, managed_pipeline in pipelines_to_check:
                try:
                    ...
                    if pipeline_id not in PIPELINE_HEALTH:
                        PIPELINE_HEALTH[pipeline_id] = {'failures': 0, 'last_check': time.time(), 'marked_for_removal': False}
                    ...
                    try:
                        process_ram_usage_mb = app._get_process_memory_usage_mb(process=process)
                        managed_pipeline.ram_usage_queue.append(process_ram_usage_mb)
                        total_ram_usage += process_ram_usage_mb
                    except Exception as e:
                        logger.warning(f"Failed to get RAM usage for {pipeline_id}: {e}")
                    ...
                    health_check_success = perform_safe_health_check(pipeline_id, managed_pipeline)
                    ...
                    if PIPELINE_HEALTH[pipeline_id]['failures'] >= MAX_HEALTH_FAILURES:
                        logger.error(f"Pipeline {pipeline_id} failed too many health checks, removing")
                        pipelines_to_remove.append(pipeline_id)
...
def patched_ensure_idle_pipelines_warmed_up(expected_warmed_up_pipelines: int) -> None:
    ...
    logger.info(f"Starting patched warm-up daemon for {expected_warmed_up_pipelines} pipelines")
    ...
    while not SHUTDOWN_EVENT.is_set():
        try:
            with app.PROCESSES_TABLE_LOCK:
                ...
                idle_pipelines = len([
                    pipeline_id for pipeline_id, managed_pipeline in app.PROCESSES_TABLE.items()
                    if managed_pipeline.is_idle and not PIPELINE_HEALTH.get(pipeline_id, {}).get('marked_for_removal', False)
                ])
                
            if idle_pipelines < expected_warmed_up_pipelines:
                pipeline_id = app.spawn_managed_pipeline_process(processes_table=app.PROCESSES_TABLE)
                if pipeline_id:
