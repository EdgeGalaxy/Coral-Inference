...
class TimeBasedVideoSink:
    ...
    def __init__(
        ...
        logger.info(
            f"TimeBasedVideoSink initialized for pipeline {pipeline_id}, video_info: {self.video_info}, queue_size: {queue_size}"
        )
    ...
    def _create_new_segment(self, timestamp: datetime):
        ...
        if self.current_writer is not None:
            ...
            if self.current_segment_path and os.path.exists(self.current_segment_path):
                ...
                logger.info(
                    f"Video segment created and optimized: {self.current_segment_path}, size: {file_size} bytes"
                )
    ...
    def _ensure_writer_initialized(self, image: np.ndarray):
        ...
        if (
            self.video_info
            and hasattr(self.video_info, "fps")
            and self.video_info.fps > 0
        ):
            ...
            dynamic_fps = (
                self.measured_fps
                if self.measured_fps and self.measured_fps > 0
                else 10.0
            )
            # 合理约束范围，避免编码器异常
        ...
        self.current_writer = cv2.VideoWriter(
            self.current_segment_path, fourcc, self.actual_fps, self.actual_resolution
        )

        logger.info(
            f"VideoWriter initialized: {self.actual_resolution} @ {self.actual_fps}fps"
        )
        ...
        try:
            ...
            for filename in os.listdir(self.output_directory):
                ...
                try:
                    ...
                    logger.warning(
                        f"Skip existing video file due to error: {path}, err: {e}"
                    )
            if loaded_count:
                logger.info(
                    f"Loaded {loaded_count} existing video files from {self.output_directory}"
                )
        except Exception as e:
    ...
    def _cleanup_oldest_files(self):
        ...
        while self.video_files and (
            self.total_size > self.max_total_size * 0.9  # 清理到90%以下
            or len(self.video_files) > 100  # 最多保留100个文件
        ):
    ...
    def on_prediction(
        ...
        try:
            if video_frames is None:
                logger.warning(
                    f"pipeline {self.pipeline_id} catch video_frames is None"
                )
                return
            ...
            try:
                ...
                logger.warning(
                    f"Prediction queue full, dropping frame for pipeline {self.pipeline_id}"
                )
    ...
    def _extract_image_from_prediction(
        ...
        try:
            ...
            if self.current_writer is not None:
                try:
                    self.current_writer.release()
                    self.current_writer = None
                    if self.current_segment_path and os.path.exists(
                        ...
                        self.total_size += file_size
                        logger.info(
                            f"[fallback] Final video segment saved and optimized: {self.current_segment_path}"
                        )
                except Exception as e:
    ...
    def _optimize_video_for_web(self, video_path: str):
        ...
        def _optimize_worker(video_path: str):
            ...
            try:
                ...
                logger.info(
                    f"Background optimizing video for web compatibility: {video_path}"
                )
                command = [
                    "ffmpeg",
                    "-i",
                ...
                logger.info(
                    f"Background video optimization successful: {final_output_path}"
                )
    ...
    def _start_worker_thread(self):
        ...
        self._worker_thread = threading.Thread(
            target=self._worker_loop, daemon=False, name=f"VideoSink-{self.pipeline_id}"
        )
    ...
    def _worker_loop(self):
        ...
        # 处理剩余队列中的数据
        logger.info(
            f"Processing remaining queue items for pipeline {self.pipeline_id}..."
        )
        remaining_items = []
        while True:
            try:
        ...
        logger.info(
            f"Video sink worker thread finished for pipeline {self.pipeline_id}"
        )
    ...
    def _monitor_queue_health(self) -> None:
        ...
        if queue_usage > 0.8:  # 超过80%使用率
            if current_queue_size - self._last_queue_size_log > 100:
                logger.warning(
                    f"Pipeline {self.pipeline_id} queue usage high: {current_queue_size}/{self._prediction_queue.maxsize} ({queue_usage:.1%})"
                )
    ...
    def _process_prediction_optimized(
        ...
        try:
            ...
            if len(frames) > 1:
                ...
                for frame, prediction in zip(frames, preds):
                    ...
                    if image is not None:
                        ...
                        show_frames[frame.source_id or f"source_{len(show_frames)}"] = (
                            image
                        )
                ...
                if show_frames:
                    try:
                        ...
                        logger.warning(
                            f"Frame merging failed, using individual frames: {merge_error}"
                        )
                        # 回退到单独处理每帧
                        processed_frames = []
                        for frame, prediction in zip(frames, preds):
                            ...
                            image = self._extract_image_from_prediction(prediction)
                            image = (
                                image if isinstance(image, np.ndarray) else frame.image
                            )
                            if image is not None:
            ...
            logger.error(
                f"Error in TimeBasedVideoSink._process_prediction_optimized: {e}"
            )
