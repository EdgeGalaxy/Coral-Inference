...
def _extract_fields_from_prediction(
    pred: Optional[dict], selected_fields: List[str]
) -> Dict[str, Any]:
    if not pred:
        return {}
    result: Dict[str, Any] = {}
...
class MetricSink:
    ...
    def __init__(
        ...
        if not (
            INFLUXDB_METRICS_TOKEN
            and INFLUXDB_METRICS_URL
            and INFLUXDB_METRICS_DATABASE
        ):
            logger.warning(
            ...
            if self._enabled:
                ...
                if self._worker_thread and self._worker_thread.is_alive():
                    ...
                    if self._worker_thread.is_alive():
                        logger.warning(
                            f"MetricSink worker thread did not finish within timeout"
                        )
    ...
    def on_prediction(
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Metrics queue full, dropping metrics for pipeline {self._pipeline_id}"
                )
    ...
    def _start_worker_thread(self):
        ...
        logger.info(
            f"Started metrics sink worker thread for pipeline {self._pipeline_id}"
        )

    def _worker_loop(self):
        """后台工作线程主循环 - 批处理指标数据"""
        logger.info(
            f"Metrics sink worker thread started for pipeline {self._pipeline_id}"
        )
        ...
        # 处理剩余队列中的数据
        logger.info(
            f"Processing remaining metrics queue items for pipeline {self._pipeline_id}..."
        )
        remaining_items = []
        while True:
            try:
        ...
        logger.info(
            f"Metrics sink worker thread finished for pipeline {self._pipeline_id}"
        )
    ...
    def _create_points_from_item(self, queue_item: Dict) -> List[Point]:
        ...
        try:
            ...
            for single_frame, single_pred in zip(frames, preds):
                ...
                # 从配置中提取字段
                fields.update(
                    _extract_fields_from_prediction(single_pred, self._selected_fields)
                )
                # 补充 duration 字段（纳秒）
