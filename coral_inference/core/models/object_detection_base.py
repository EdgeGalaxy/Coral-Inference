...
from coral_inference.core.env import CURRENT_RKNN_PLATFORM, DEFAULT_RKNN_IMAGE_SIZE
from coral_inference.core.models.rknn_base import RknnCoralInferenceModel


class ObjectDetectionBaseRknnCoralInferenceModel(RknnCoralInferenceModel):
    """Coral RKNN Object detection model. This class implements an object detection specific infer method."""
    ...
    def __init__(self, model_id: str, *args, **kwargs):
        ...
        image_size = kwargs.get("image_size", DEFAULT_RKNN_IMAGE_SIZE)
        platform = kwargs.get("platform", CURRENT_RKNN_PLATFORM)
        super().__init__(
            model_id,
    ...
    def predict(self, img_in: np.ndarray, **kwargs) -> Tuple[np.ndarray]:
        """Runs inference on the RKNN model.

        Args:
            img_in (np.ndarray): The preprocessed image(s) to run inference on.

        Returns:
            Tuple[np.ndarray]: The RKNN model predictions.
