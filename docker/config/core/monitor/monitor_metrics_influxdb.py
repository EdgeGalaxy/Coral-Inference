...
class DataValidator:
    ...
    def validate_pipeline_report(report: Dict) -> bool:
        ...
        if not isinstance(latency_reports, list) or not isinstance(sources_metadata, list):
            return False
...
"""
    ...
    def __init__(self, client: InfluxDBClient3, max_retries: int = 3, 
                 retry_delay: float = 5, health_check_interval: float = 60):
        self.client = client
...
class InfluxDBMetricsCollector:
    ...
    def _init_influxdb_client(self):
        ...
        for retry in range(max_retries):
            try:
                ...
                logger.error(f"初始化 InfluxDB3 客户端失败 (重试 {retry + 1}/{max_retries}): {e}")
    ...
    async def collect_and_store_metrics_concurrent(
        self, 
        pipeline_ids_mapper: Dict[str, str]
    ):
        ...
        async with self.semaphore:
            try:
                ...
                valid_latency_reports = [
                    r for r in latency_reports if self.validator.validate_latency_report(r)
                ]
                valid_sources_metadata = [
                    m for m in sources_metadata if self.validator.validate_source_metadata(m)
                ]
                ...
                if self.pipeline_cache:
                    ...
                    if cache_info:
                        pipeline_name = cache_info.get("pipeline_name", pipeline_cache_id)
    ...
    def _create_influxdb_points(
        ...
        pipeline_point = pipeline_point.field("throughput", str(round(float(inference_throughput), 2)))
        ...
        for source_metadata in sources_metadata:
            ...
            if latency_data:
                ...
                if "latency_mean" in latency_data:
                    point = point.field("latency_mean", str(round(float(latency_data["latency_mean"]), 2)))
                if "latency_p50" in latency_data:
                    point = point.field("latency_p50", str(round(float(latency_data["latency_p50"]), 2)))
                if "latency_p90" in latency_data:
                    point = point.field("latency_p90", str(round(float(latency_data["latency_p90"]), 2)))
                if "latency_p99" in latency_data:
                    point = point.field("latency_p99", str(round(float(latency_data["latency_p99"]), 2)))
                ...
                if "fps" in latency_data:
                    point = point.field("fps", str(round(float(latency_data["fps"]), 2)))
                                ...
                                if '=' in field:
                                    ...
                                    try:
                                        if value.endswith('i'):
                                            ...
                                            point_dict["fields"][key] = str(round(float(value), 2))
                                        else:
