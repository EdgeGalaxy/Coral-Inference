...
class OptimizedResultsCollector:
    ...
    def __init__(self, stream_manager_client: Optional[StreamManagerClient], output_dir: Path,
                 batch_size: int = 100, flush_interval: float = 30,
    ...
    async def poll_and_save_results_concurrent(self, pipeline_ids_mapper: Dict[str, str]):
        """并发轮询多个 pipeline 的结果"""
...
class OptimizedCleanupManager:
    ...
    def __init__(self, output_dir: Path, max_days: int = 7, 
    ...
    async def _cleanup_by_size_async(self):
        ...
        try:
            ...
            for dir_info in pipeline_dirs:
                ...
                logger.info(f"清理目录: {dir_info['path']}, 释放 {dir_info['size']:.2f} GB")
...
class OptimizedPipelineMonitorWithInfluxDB:
    ...
    def __init__(
        ...
        self._validate_parameters(
            poll_interval, max_days, cleanup_interval, 
            results_batch_size, max_size_gb, max_background_workers
        )
    ...
    def _validate_parameters(self, poll_interval: float, max_days: int, 
                           cleanup_interval: float, batch_size: int, 
                           max_size_gb: float, max_workers: int):
        """验证初始化参数"""
        if poll_interval <= 0:
    ...
    async def _run_monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                tasks = [
                    # 结果收集
                    self.results_collector.poll_and_save_results_concurrent(pipeline_ids_mapper),
                    # 清理管理
                ...
                if self.influxdb_collector:
                    tasks.append(
                        self.influxdb_collector.collect_and_store_metrics_concurrent(pipeline_ids_mapper)
                    )
    ...
    async def _cleanup(self):
        ...
        # 刷新所有缓存
        cleanup_tasks = [
            self.results_collector.flush_all_caches()
        ]
    ...
    async def get_performance_metrics(self) -> Dict:
        ...
        if self.influxdb_collector:
            metrics['influxdb_buffer_size'] = len(self.influxdb_collector.metrics_buffer)
